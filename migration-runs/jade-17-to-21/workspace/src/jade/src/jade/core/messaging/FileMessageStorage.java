	...
	public synchronized void loadAll(LoadListener ll) throws IOException {
		...
		for(int i = 0; i < subdirs.length; i++) {
			...
			for(int j = 0; j < files.length; j++) {
				...
				BufferedReader in = new BufferedReader(new FileReader(toRead, StandardCharsets.UTF_8));
...
// JADE-MODERNIZATION-DEFERRED:TRY_WITH_RESOURCES Extremely broad pattern (1832 flags), deferred for targeted future review
	...
	private void incrementCounter(File f) throws IOException {
		BufferedReader in = new BufferedReader(new FileReader(f, StandardCharsets.UTF_8));
		File tmp = File.createTempFile("JADE", ".tmp");
		String s = in.readLine();
		try {
// JADE-MODERNIZATION-DEFERRED:TRY_WITH_RESOURCES Extremely broad pattern (1832 flags), deferred for targeted future review
			long counter = Long.parseLong(s);
			BufferedWriter out = new BufferedWriter(new FileWriter(tmp, StandardCharsets.UTF_8));
			try {
// JADE-MODERNIZATION-DEFERRED:TRY_WITH_RESOURCES Extremely broad pattern (1832 flags), deferred for targeted future review
				counter++;
...
// JADE-MODERNIZATION-DEFERRED:TRY_WITH_RESOURCES Extremely broad pattern (1832 flags), deferred for targeted future review
			...
			out = new BufferedWriter(new FileWriter(toStore, StandardCharsets.UTF_8));
