...
// JADE-FLAG:JEP400_FILE_IO_DEFAULT_CHARSET java.io FileReader/FileWriter constructor with no Charset argument; JEP 400 changed it from the platform default to UTF-8 in JDK 18 0.95
		...
		Action saveAction = new AbstractAction("Save", saveImg){
	  	public void actionPerformed(ActionEvent e)
	  	{
			  ...
			  if(returnVal == JFileChooser.APPROVE_OPTION)
			  {
				  ...
				  try {
// JADE-MODERNIZATION-DEFERRED:TRY_WITH_RESOURCES Extremely broad pattern (1832 flags), deferred for targeted future review
				    FileWriter f = new FileWriter(fileName, StandardCharsets.UTF_8);
            // Note the save/read functionality uses default US-ASCII charset
	  ...
	  public void actionPerformed(ActionEvent e)
	  {
	  	JFileChooser chooser = new JFileChooser(); 
			...
			if(returnVal == JFileChooser.APPROVE_OPTION)
			{
				...
				try
				{
					BufferedReader inp = new BufferedReader(new FileReader(fileName, StandardCharsets.UTF_8));
