...
// JADE-MODERNIZATION-DEFERRED:TRY_WITH_RESOURCES Extremely broad pattern (1832 flags), deferred for targeted future review
	...
	/**
	 * Release the listening socket, the client socket and the connection
	 * thread. Called from the finally block of run(), so it runs on the normal
	 * path; it used to be the finalizer, which JEP 421 deprecated for removal
	 * in JDK 18, and which run() already had to call explicitly to get this
