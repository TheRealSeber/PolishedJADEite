...
DUMMY_RECIPE = ".claude/skills/java-migration-skill-registry/shared/jade-recipe-dummy/scripts/apply.py"
...
def test_validate_flag_rejects_missing_string_zero_and_negative_lines():
    module = load_module()

    for flag in ({}, {"rule_id": "RULE", "file": "Example.java", "line": "4"},
                 {"rule_id": "RULE", "file": "Example.java", "line": 0},
                 {"rule_id": "RULE", "file": "Example.java", "line": -1},
                 {"rule_id": "RULE", "file": "", "line": 1},
                 {"rule_id": "", "file": "Example.java", "line": 1},
                 {"rule_id": "OTHER", "file": "Example.java", "line": 1}):
        error = module.validate_flag(flag, "RULE")
        assert error


def test_validate_flag_accepts_positive_integer_line():
    module = load_module()

    flag = {"rule_id": "RULE", "file": "Example.java", "line": 4}
    assert module.validate_flag(flag, "RULE") is None
...
def test_dispatch_recipe_returns_failed_json_for_missing_script(tmp_path):
    ...
    missing = ".claude/skills/java-migration-skill-registry/shared/missing/scripts/apply.py"
    result = module.dispatch_recipe(
        missing,
        str(workspace_file),
        1,
    ...
    assert result == {
        ...
        "changes": 0,
        "warnings": [],
        "errors": [f"Recipe script not found: {missing}"],
        "diff_summary": f"Recipe script not found: {missing}",
    }
...
def test_dispatch_recipe_handles_oserror(tmp_path, monkeypatch):
    ...
    result = module.dispatch_recipe(DUMMY_RECIPE, str(tmp_path / "Example.java"), 1)
    assert result["status"] == "FAILED"
...
def test_main_rejects_workspace_traversal_without_dispatch(tmp_path, monkeypatch, file_rel):
    ...
    monkeypatch.setattr(module, "load_registry", lambda: {
        "RULE": {"skill": "recipe", "script": DUMMY_RECIPE}
    })
...
@pytest.mark.parametrize(
    ...
    [
        f"./{DUMMY_RECIPE}",
        ".claude//skills/java-migration-skill-registry/shared/jade-recipe-dummy/scripts/apply.py",
    ],
)
...
def test_dispatch_recipe_accepts_valid_result(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(subprocess, "run", lambda *args, **kwargs: subprocess.CompletedProcess(args[0], 0, json.dumps(payload), ""))
    assert module.dispatch_recipe(DUMMY_RECIPE, str(tmp_path / "Example.java"), 1) == payload
